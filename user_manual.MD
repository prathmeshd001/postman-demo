...
pm.test("Saved demoValue", () => pm.expect(pm.environment.get("demoValue")).to.exist);
```
...
```js
pm.visualizer.set(`<table border="1" cellpadding="6">
<tr><th>Demo Value</th><td>{{val}}</td></tr>
</table>`, { val: pm.environment.get("demoValue") });
```
